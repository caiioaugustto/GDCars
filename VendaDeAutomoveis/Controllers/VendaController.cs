...
namespace VendaDeAutomoveis.Controllers
{
    ...
    public class VendaController : Controller
    {
        ...
        {
            model.Clientes = clienteRepository.ObterTodos();
            model.Vendas = vendaRepository.BuscarPorCliente(model.IdCliente);
            return View(model);
        }

        private void MudarClienteComunParaVip(Cliente cliente)
        {
            //if (cliente.TipoDoCliente == TipoCliente.Comum && vendaRepository.TotalPorCliente(cliente.IdCliente) >= 200000)
            //{
